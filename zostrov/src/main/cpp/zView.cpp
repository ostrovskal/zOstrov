...
void zView::_requestLayout() {
    status &= ~ZS_MEASURE;
    if(parent && parent->testFlags(ZS_MEASURE))
        parent->_requestLayout();
}

void zView::_requestPosition() {
    status &= ~ZS_LAYOUT;
    if(parent && parent->testFlags(ZS_LAYOUT))
        parent->_requestPosition();
}
...
void zView::_invalidate() {
    status |= ZS_DIRTY_LAYER;
    if(parent && parent->isInvalidate())
        if(parent != this) parent->_invalidate();
}
