...
void zViewText::drawText() {
    ...
    if(textCache.isNotEmpty()) {
        ...
        //rti clip(z_clipRect(rclip, rclient));
...
/*
    if(cacheSpans.size() > 1) {
        for(auto &s : cacheSpans) {
            DLOG("span: %s", s->info().str());
        }
    }
*/
}
