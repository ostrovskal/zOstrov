...
zStyle* zTheme::getStyles(u32 _value) const {
...
const zStyle* zStyle::findAttribute(u32 _attr) const {
    auto cur(this); u32 m(_attr & ZTA_MASK);
    do {
        auto _a(cur->a & ZTA_MASK);
        if(_a == Z_STYLES) {
            auto styles(theme->getStyles(cur->v));
            if(styles) {
                auto ret(styles->findAttribute(_attr));
                if(ret) return ret;
            }
        } else {
            if(m == _a)
                return resolveAttribute(cur);
        }
    } while(!(cur++->a & ZT_END));
    // не удалось найти атрибут при поиске в теме или
    // при непосредственном поиске атрибута определенного типа
    return nullptr;
}
...
void zStyle::enumerate(const std::function<void(u32)>& act) const {
    ...
    do {
        ...
        if(_a == Z_STYLES) {
            auto styles(theme->getStyles(_style->v));
